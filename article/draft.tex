...
It is of course not surprising that plaforms are regulating their content, sometimes by removing it, but we would argue that it could simplify the researchers' work if the deleted data could still be accessible in the APIs (Pasquetto, Swire-Thompson, Amazeen et al. (2020)~\cite{pasquetto}).
...
Thus, counting specific hashtag use over time allows to indirectly observe permanent suspensions on Twitter, and other similar methodologies can be found for the other platforms.
...
{\color{brown} A last concern concerns the increasing restrictions to access data via APIs.

1/ Restrictions to the Open Graph API with the Cambridge Analytica scandal. 

2/ Now CT exists but doubt were recently raised that this API will still be accessible for researchers and journalists.

3/ Still access to the "official" APIs for Twitter and YouTube, although the access to Twitter API is very hard to justify if we say we are researchers.

4/ Increasing volume restrictions on the Twitter API between V1 and V2.

It should be noted that there are other ways to collect data than directly from the platforms' APIs. Indeed, other APIs are also aggregating data from multiple social platforms as there is a need for social marketing. CrowdTangle was actually first an independent social listening tool, before being bought and included within the Facebook organization. For example Newsguard, Buzzsumo, Linkfluence... Problem: often it is not clear how the data are collected, and at which frequency (more reliable to go to the "source", i.e. the official API).}
...
{\color{pink} Make point / Recommendation: inform about sources, example inform users that this user/account/page/group/channel has shared x failed fact-checks over a x-months period.}
%\smallskip
