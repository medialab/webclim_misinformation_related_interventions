...
Finally, we are aware that full transparency regarding how misinformation related interventions are operated, can backfire, as the prospective aimed users might find means to circumvent the partly automated interventions. To provide an example, it is very likely that YouTube is allocating an information panel on all YouTube videos mentioning COVID-19 and related terms in their title (see section \ref{youtube_panels}). Yet, we noticed during the present study, that some YouTube channels are trying to avoid the likely automated information panels, by using notational variants such as ``C.O.V.I.D'' or ``C O V I D''.  Hence, a balance is needed between fully transparent communication about misinformation related interventions so the users can know precisely, how the platforms are regulating their content, and not explicitly giving too much information which would allow the policies to be bypassed.
