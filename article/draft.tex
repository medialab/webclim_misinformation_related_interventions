...
[CONCLUSION]
This drop in views can show that the  suspension  period may  have  a  good impact on reducing  the  audience interest or reach to the channel.
...
YouTube may provide an information panel for videos with topics that are prone to misinformation like COVID-19, moon landing, and climate change. 
Information panels provide ressources concerning a potentially controversial topic from independent third parties or authoritative ressources {\color{red} [LIKE blablabla]}. 
Importantly, YouTube states that these panels exist regardless of the point of view expressed in a given video. {\color{red} [REF??]}
%It should be noted that these panels are not yet available in all languages and countries.
...
We collected the information panels, when present, in each video by connecting to a US server and scraping the content of the web page. 
Five types of information panels were found in the list of visited videos regarding climate change, COVID-19 Vaccine, COVID-19, vaccine hesitancy and US elections, as shown in figure \ref{youtube_panel_screenshots}.
{\color{red} [ADD Vaccine hesitancy screenshot + REARRANGE SCREENSHOT IN FREQUENCY ORDER]}
...
We found that almost half (44 \%) of the videos containing misinformation actually had no information panel (Table \ref{youtube_panel_proportions}).
For the rest of videos, the most frequent information panels found concerned COVID-19 (42 \%) and COVID-19 vaccine (11 \%).
...
\hline
\multicolumn{6}{|c|}{$170$ YouTube videos marked as false or inaccurate}                                                                                                                                                                                                                                                                                                                           \\ \hline
...
\makecell{With a \\ `vaccine hesitancy' \\ panel}  
\\ \hline
\makecell{74 \\ (44 \%)} &
\makecell{71 \\ (42 \%)} &
\makecell{19 \\ (11 \%)} &
...
In our list, we noticed that the same video was sometimes uploaded multiple times and thus appeared on different YouTube links. 
...
Note that in figure \ref{duplicates_yt}, the video with `C.O.V.I.D.19' in its title had an information panel, hinting that YouTube might adapt its list of `suspect' keywords as new notational variants for COVID-19 appear.
